...
## 1.3.0 (2026-09-15) — Mode D Websites: Audit + Oferta 2026 + Batch Leads

### Added
- `ai_monetize/websites/audit.py` — auditoria determinista de URL (3 defectos que cuestan llamadas/leads) + outreach basado en evidencia.
- `--audit`, `--audit-business`, `--audit-lang` flags en CLI para correr audit y generar texto listo para copiar.
- `ai_monetize/websites/leads.py` — batch por CSV: genera bundles por lead (opcionalmente corre audit si hay columna url).
- `--leads-file`, `--leads-out`, `--leads-audit` flags en CLI para batch de websites.
- `ai_monetize/websites/tracker.py` — tracker simple (cadencia 7/7/3) + flags CLI: `--tracker-add`, `--tracker-touch`, `--tracker-summary`.
- `docs/MODES.md` — definicion actual de modos + mapeo legacy.

### Changed
- Mode D oferta: modelo 2026 (sin fee de build, sin contrato, cancel anytime, cliente dueno del sitio) + tiers LATAM/US + upsell AI Receptionist (solo documentado).
- Websites generator: soporte basico de idioma (es/en), tema (dark/light), schema JSON-LD (LocalBusiness + FAQ), formulario de cotizacion a WhatsApp, y multi-pagina en tier Growth.
- `stages/analytics.py` (Mode D): TRACKING.md enfocado a dinero influido (leads -> cierres).
- Version bump: 1.0.2 -> 1.3.0 (pyproject + ai_monetize.__version__).

## 1.2.0 (2026-08-09) — Consistencia de Video (3 técnicas)

### Added
- `docs/faceless-methodology.md` v2.1.0 — sección "3 Técnicas de Consistencia de Video": (1) Character Consistency via Seedance 2.5 R2V (50 refs, character sheet), (2) Scene/World Consistency via Google Flow + world plates + keyframes, (3) Voice Consistency via voice blocks (Voicebox/ElevenLabs). Orquestación pipeline Modo A + regla de oro (consistencia es propiedad de la serie, no del clip)

### Changed
- `docs/faceless-methodology.md` — version 2.0.0 -> 2.1.0, fuentes actualizadas con transcripts 2026-08-03/05 (fMU7Ku8ma4A, 3h89eX-p5sE, JWeSMMMJbts)

## 1.1.0 (2026-07-30) — Guias Operativas + Anti-Deteccion

### Added
- `guides/operations/01_IP_ANTI_DETECTION.md` — Guia completa anti-deteccion: senales por plataforma, tipos de IP (VPN/datacenter/residencial/mobile), browser fingerprinting (Canvas/WebGL/Audio/fonts), content fingerprint evasion, arquitectura recomendada por cuenta, pipeline de creacion segura, warm-up 7-14 dias, health monitoring, costos de infraestructura ($50-299/mes)
- `guides/operations/02_OPERATIONAL_MODELS.md` — Comparativa exhaustiva de 4 modelos: Freelancer (CaaS $10-50/clip, caso verificado $24,859/mes), Solo Operator (canales propios, $10-50K+/mes), Team/Agency (contratacion VAs, SOPs), Hibrido (recomendado, revenue mixing $500-22K/mes). Plan de 12 meses detallado
- `guides/operations/03_PLATFORM_RESTRICTIONS.md` — Politicas 2026 por plataforma: TikTok (Creative Health Scores, limites, shadowban triggers), YouTube (YPP, AI disclosure, cross-account enforcement), Instagram/Meta (Business Manager linking, cross-platform graph), Reddit, X/Twitter. Estrategia multi-plataforma segura
- `guides/operations/04_NICHE_STRATEGY.md` — Estrategia por nicho: Green Grid top 10 scoring, high vs low RPM, evergreen vs trending, estrategia dual (cuentas propias en high RPM + freelance en low RPM), ejemplo completo para AI Tools
- `PROMPT.md` — v1.1.0: tabla de guias operativas, seccion anti-deteccion como capa externa obligatoria, gaps documentados en ai-monetize accounts module, nueva decision 2026-07-30

### Changed
- `PROMPT.md` — version bump 1.0.3 -> 1.1.0, nuevas reglas NO HACER (multi-cuenta sin anti-deteccion, cross-post identico)

## 1.0.3 (2026-07-29) — Produccion + Integraciones Externas

### Added
- `integrations/postiz/client.py` — PostizClient con schedule_post, list_posts, analytics, health
- `integrations/openmontage/client.py` — OpenMontaceClient con 12 pipelines via CLI subprocess
- `publish/providers/reddit.py` — RedditProvider (OAuth2, text/link/media, comments, metrics)
- `publish/providers/twitter.py` — TwitterProvider (OAuth 2.0, media upload chunked, reply, metrics)
- `utils/logging_config.py` — Logger centralizado JSON/rotacion, JSONFormatter, vars LOG_DIR/LOG_LEVEL/LOG_FORMAT
- `web/api.py` — CORS middleware, `/health` endpoint (DB/Redis/Celery/backends/platforms), `/monitor` dashboard HTML
- `web/server.py` — setup_logging() al inicio, banner con link a /monitor
- `scripts/run_tests.sh` — Runner auto-venv: crea venv si falta, instala extras, ejecuta pytest
- CI/CD — lint stage (ruff), docker stage (build + verify)
- Dockerfile — HEALTHCHECK contra /health, /var/log/ai-monetize
- `.env.template` — REDDIT_* y TWITTER_* variables
- 14 nuevos tests (Reddit + Twitter), 51 total publish tests

### Changed
- `publish/providers/__init__.py` — reddit + twitter registrados en PROVIDER_REGISTRY (5 providers total)
- `pyproject.toml` — version sync 1.0.2
- `.github/workflows/test.yml` — usa `scripts/run_tests.sh`, evita test_db.py en CI
- `docker-compose.yml` — healthcheck actualizado a /health
- Docstrings agregados en todos los providers, types.py, exceptions.py

### Fixed
- `render/backends.py` — leak parcial de API key ([:8]) reemplazado por "[configured]"
...
- Modo D (legacy "Auto-Pilot") integrado con multi-cuenta: `--multi` genera N cuentas con nivel auto
